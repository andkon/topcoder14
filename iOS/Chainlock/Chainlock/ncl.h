...
\defgroup negotiation Negotiation
	...
	and validation allows the  \ref NEA to recognize a previously provisioned Nymi.
...
typedef enum{
	...
	NCL_MODE_SYNCH=1<<0,///<\brief Callbacks will be called from inside \ref nclUpdate, which must be called regularly. If not set, callbacks happen when an event occurrs, on their own thread.
...
typedef struct{ char string[512]; } NclInfo;///< \brief NCL type for sending info. See \ref nclInfo. \ingroup coreTypes
